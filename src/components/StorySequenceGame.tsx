...
export default function StorySequenceGame() {
  const [placedCards, setPlacedCards] = useState<{ [key: number]: StoryImage | null }>({});
  const [selectedCard, setSelectedCard] = useState<StoryImage | null>(null);
  const [gameComplete, setGameComplete] = useState(false);
  const [resultMessage, setResultMessage] = useState('');
  ...
  // initialize board
  useEffect(() => {
    const initial: { [key: number]: StoryImage | null } = {};
    for (let i = 1; i <= 9; i++) initial[i] = null;
    setPlacedCards(initial);
  }, []);
  ...
  const playResult = (success: boolean) => {
    const s = success ? successSoundRef.current : errorSoundRef.current;
    if (s) {
      s.currentTime = 0;
      s.play().catch(() => {});
    }
  ...
  const handleCardClick = (image: StoryImage) => {
    playClick();
    setSelectedCard(prev => (prev?.id === image.id ? null : image)); // toggle selection
  };

  const handleTileClick = (position: number) => {
    if (selectedCard && !placedCards[position]) {
      // place selected card
      const newPlaced = { ...placedCards, [position]: selectedCard };
      setPlacedCards(newPlaced);
      setSelectedCard(null);

      // check if complete
      const count = Object.values(newPlaced).filter(c => c !== null).length;
      if (count === 9) checkSequence(newPlaced);
    }
  };

  const handlePlacedCardClick = (position: number) => {
    // take card back to available zone
    playClick();
    const card = placedCards[position];
    if (!card) return;
    const newPlaced = { ...placedCards, [position]: null };
    setPlacedCards(newPlaced);
    setSelectedCard(card); // optional: immediately select it for placing somewhere else
  };
  ...
  const checkSequence = (cards: { [key: number]: StoryImage | null }) => {
    ...
    setGameComplete(true);
    if (isCorrect) {
      setResultMessage('🎉 Excellent! You arranged the story correctly!');
      playResult(true);
    } else {
      setResultMessage('❌ Sequence is incorrect. Try again!');
      playResult(false);
    }
  };
  ...
  const availableImages = storyImages.filter(
    img => !Object.values(placedCards).some(c => c?.id === img.id)
  );
  ...
  return (
    <div className="game-container">
      ...
      <header className="text-center mb-8">
        <h1 className="story-title mb-4">Spiritual Journey of Ramakrishna Paramahamsa</h1>
        <p className="text-lg text-muted-foreground">
          Tap a card to select it, then tap a square to place it in order!
        </p>
      </header>
      ...
      <div className="board-grid">
        {[1,2,3,4,5,6,7,8,9].map(position => (
          <div
            key={position}
            className={`drop-zone ${placedCards[position] ? 'occupied' : ''}`}
            onClick={() => placedCards[position]
              ? handlePlacedCardClick(position)
      ...
      <div className="images-container">
        ...
        <div className="images-grid">
          {availableImages.map(image => (
            <div
              key={image.id}
              className={`story-card ${selectedCard?.id === image.id ? 'selected' : ''}`}
